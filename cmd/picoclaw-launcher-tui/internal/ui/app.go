...
import (
	...
	configstore "github.com/strand1/fernwood/cmd/picoclaw-launcher-tui/internal/config"
...
func refreshMainMenu(menu *Menu, s *appState) {
	...
	items := []MenuItem{
		...
		{
			Label:       "Start Talk",
			Description: "Open fernwood agent in terminal",
			Action: func() {
				s.requestStartTalk()
			},
			Disabled: !modelReady,
...
func (s *appState) startTalk() {
	...
	s.app.Suspend(func() {
		cmd := exec.Command("fernwood", "agent")
		cmd.Stdin = os.Stdin
...
func (s *appState) startGateway() {
	...
	cmd := exec.Command("fernwood", "gateway")
