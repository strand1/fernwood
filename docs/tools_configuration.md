...
Fernwood's tools configuration is located in the `tools` field of `config.json`.
...
By default, Fernwood blocks the following dangerous commands:
...
All configuration options can be overridden via environment variables with the format `FERNWOOD_TOOLS_<SECTION>_<KEY>`:

For example:

- `FERNWOOD_TOOLS_WEB_BRAVE_ENABLED=true`
- `FERNWOOD_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `FERNWOOD_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
- `FERNWOOD_TOOLS_MCP_ENABLED=true`
